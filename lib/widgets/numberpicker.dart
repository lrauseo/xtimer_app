...
class NumberPicker extends StatelessWidget {
  ...
  NumberPicker.decimal(
      ...
      : assert(initialValue != null),
        ...
        selectedDecimalValue =
            ((initialValue - initialValue.floorToDouble()) * math.pow(10, decimalPlaces)).round(),
        intScrollController = new ScrollController(
          initialScrollOffset: (initialValue.floor() - minValue) * itemExtent,
        ),
        decimalScrollController = new ScrollController(
          initialScrollOffset:
              ((initialValue - initialValue.floorToDouble()) * math.pow(10, decimalPlaces)).roundToDouble() *
                  itemExtent,
        ),
        _listViewHeight = 3 * itemExtent,
        step = 1,
  ...
  Widget _integerListView(ThemeData themeData) {
    ...
    TextStyle selectedStyle = themeData.textTheme.headline1!.copyWith(color: themeData.accentColor);
      ...
      child: new Container(
        ...
        child: new ListView.builder(
          ...
          itemBuilder: (BuildContext context, int index) {
            ...
            final TextStyle itemStyle = value == selectedIntValue ? selectedStyle : defaultStyle;
  ...
  Widget _decimalListView(ThemeData themeData) {
    ...
    int itemCount = (selectedIntValue == maxValue ? 3 : math.pow(10, decimalPlaces) + 2) as int;
  ...
  int _normalizeDecimalMiddleValue(int decimalValueInTheMiddle) {
    return _normalizeMiddleValue(decimalValueInTheMiddle, 0, (math.pow(10, decimalPlaces) - 1) as int);
  }

  ///indicates if user has stopped scrolling so we can center value in the middle
  bool _userStoppedScrolling(Notification notification, ScrollController scrollController) {
    return notification is UserScrollNotification && notification.direction == ScrollDirection.idle;
...
class NumberPickerDialog extends StatefulWidget {
  ...
  State<NumberPickerDialog> createState() =>
      new _NumberPickerDialogControllerState(initialIntegerValue, initialDoubleValue);
}
