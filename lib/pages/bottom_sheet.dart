...
class _BottomSheetState extends State<BottomSheet> {
  ...
  void _handleDragUpdate(DragUpdateDetails details) {
    ...
    widget.animationController?.value -=
        details.primaryDelta ?? 0 / (_childHeight ?? details.primaryDelta ?? 0);
  }
...
class _ModalBottomSheetState<T> extends State<_ModalBottomSheet<T>> {
  ...
  Widget build(BuildContext context) {
    return new GestureDetector(
        onTap: widget.route?.dismissOnTap != null ? () => Navigator.pop(context) : null,
        child: new AnimatedBuilder(
            animation: widget.route!.animation!,
            builder: (BuildContext context, Widget? child) {
              double bottomInset = widget?.route?.resizeToAvoidBottomPadding ?? false
                  ? MediaQuery.of(context).viewInsets.bottom
                  : 0.0;
              return new ClipRect(
                  child: new CustomSingleChildLayout(
                      delegate: new _ModalBottomSheetLayout(widget.route?.animation?.value ?? 0, bottomInset),
